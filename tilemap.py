...
class Tilemap(sprite.Sprite):
    def __init__(self, map, tile_size, tileset, light_intensity=127, x=0, y=0):
        ...
        self.tileset_dark.fill(
            (light_intensity, light_intensity, light_intensity), None, pygame.BLEND_MULT
        )
    ...
    def update(self, dt, viewport_x, viewport_y):
        ...
        for x in range(
            max(0, map_x - 1), min(map_x + Settings.SCREEN_WIDTH + 1, self.map_w)
        ):
            for y in range(
                max(0, map_y - 1), min(map_y + Settings.SCREEN_HEIGHT + 1, self.map_h)
            ):
                tile = data[x][y]
